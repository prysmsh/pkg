...
func TestDecryptPayload_WrongSecret(t *testing.T) {
	kp, _ := GenerateKeyPair()
	_, secret, _ := Encapsulate(kp.PublicKey())
	ct, _ := EncryptPayload(secret, []byte("data"))

	var wrongSecret [SharedSecretSize]byte
	wrongSecret[0] = 0xff
	_, err := DecryptPayload(wrongSecret, ct)
	if err == nil {

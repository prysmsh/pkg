...
func TestRandomBytes_Zero(t *testing.T) {
	...
	if b == nil {
		t.Error("expected empty slice for n=0, got nil")
	}
	if len(b) != 0 {
...
func TestRandomBytes_Negative(t *testing.T) {
	b, err := RandomBytes(-1)
	if err != nil {
		t.Fatalf("RandomBytes(-1): %v", err)
	}
	if len(b) != 0 {
		t.Errorf("expected empty slice for n=-1, got %d bytes", len(b))
	}
}
...
func TestRandomHex_Zero(t *testing.T) {
	...
	if s != "" {
		t.Errorf("expected empty string for n=0, got %q", s)
	}
}
